...
import { optimizeResponse } from '../test/fixtures'
import { OptimizerPage } from './OptimizerPage'

const mocks = vi.hoisted(() => ({ optimize: vi.fn(), authorizeMock: vi.fn() }))
vi.mock('../api/client', () => ({
  DashboardApiError: class extends Error {},
  api: { optimize: mocks.optimize, authorizeMock: mocks.authorizeMock },
}))

describe('OptimizerPage', () => {
  beforeEach(() => {
    mocks.optimize.mockReset().mockResolvedValue(optimizeResponse)
    mocks.authorizeMock.mockReset().mockResolvedValue({ execution: { status: 'authorized', authorized_amount_paise: 74903 } })
  })
  ...
  it('changes policy through the real request and keeps execution separate', async () => {
    ...
    await waitFor(() => expect(mocks.optimize).toHaveBeenLastCalledWith(expect.objectContaining({ risk_profile: 'conservative' })))
